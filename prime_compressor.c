...
int attempt_shrink(LINT*target)
{
    ...
    {
        unsigned int*aux= (unsigned int*)realloc(target->x,new_size*sizeof(unsigned int));
        if(!aux)
        {
...
char* lint_itoa(LINT* target)
{
    ...
    char* aux=(char*)realloc(result,result_iterator+1);
    if(aux)
    {
        result=aux;
    }
...
{
    unsigned int *primes = (unsigned int *)malloc(NUM_PRIMES * sizeof(int));
    int *primesCounters = (int *)malloc(NUM_PRIMES * sizeof(int));
